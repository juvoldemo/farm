...
3. `supabase/migrations/003_crop_genetics_hybrids.sql`
4. `supabase/migrations/004_long_term_economy.sql`
5. `supabase/migrations/005_fix_legacy_harvest.sql`
6. `supabase/migrations/006_friend_crop_theft.sql`

Migration thứ hai tạo profile, farm, 24 luống, kho, tiến trình, quan hệ bạn bè, lịch sử ghé thăm, thông báo, RLS, RPC và trigger tạo dữ liệu mặc định. Migration 006 bổ sung hành động lấy 10–15% sản lượng cây chín của bạn bè, chống lấy lặp theo từng cây và trừ sản lượng của chủ vườn bằng RPC transaction.
...
5. A chọn Ghé thăm; xác nhận chỉ cây đã chín có nút “Ăn trộm”, nhận 10–15% sản lượng và không thể lấy lại cùng cây.
6. Đăng nhập B, xác nhận có thông báo cây bị lấy và sản lượng thu hoạch đã bị trừ đúng lượng A nhận.
7. Đăng xuất A, đăng nhập B trên cùng trình duyệt; xác nhận không lóe hoặc giữ state của A.
8. Mở cùng tài khoản ở mobile/web, thao tác rồi đợi trạng thái “Đã đồng bộ”.
...
- Hành động lấy nông sản chỉ chạy qua `steal_friend_crop`; client không tự cộng kho hoặc quyết định sản lượng.
