...
export const migrateSaveGame=(input:unknown):GameStateData=>{
 ...
 const plots=old.plots.map((plot,index)=>{const balance=plotUnlockConfig[index]??{price:plot.unlockPrice,requiredLevel:plot.requiredLevel};if(!plot.cropInstance)return{...plot,unlockPrice:balance.price,requiredLevel:balance.requiredLevel}
  ...
  return{...plot,unlockPrice:balance.price,requiredLevel:balance.requiredLevel,cropInstance:{...plot.cropInstance,plantedAt:new Date(plantedAt).toISOString(),readyAt:new Date(readyAt).toISOString(),baseGrowthDuration:crop?.growthDurationSeconds??plot.cropInstance.baseGrowthDuration,rarity:plot.cropInstance.rarity??'common',traits:plot.cropInstance.traits??[],generation:plot.cropInstance.generation??0,calculatedGrowthDuration:crop?.growthDurationSeconds??plot.cropInstance.calculatedGrowthDuration??plot.cropInstance.baseGrowthDuration,weatherEffects:plot.cropInstance.weatherEffects??{rainExperienced:false},care:plot.cropInstance.care??{water:50,weeds:false,pests:false}}}
 })
