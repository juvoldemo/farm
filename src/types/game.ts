...
export interface FarmOrder { id:string; customerName:string; customerAvatar?:string; items:FarmOrderItem[]; rewardGold:number; rewardXp:number; rewardDiamonds?:number; createdAt:string; expiresAt?:string; rarity:'normal'|'rare'|'special'; status:'active'|'completed'|'claimed'|'expired' }
